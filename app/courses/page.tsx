...
const mockCourses: Course[] = [
  {
    ...
    description: 'Comprehensive course covering React, Node.js, MongoDB, and deployment strategies for modern web applications.',
    institute: {
      id: 'inst1',
  ...
  },
  {
    ...
    description: 'Introduction to data analysis, statistics, machine learning, and Python programming for data science.',
    institute: {
      id: 'inst3',
  ...
  },
  {
    ...
    description: 'Build cross-platform mobile applications using React Native, covering iOS and Android development.',
    institute: {
      id: 'inst1',
      name: 'Tech Academy Institute',
      logo: 'https://images.unsplash.com/photo-1599305445671-ac291c95aaa9?w=100&h=100&fit=crop&crop=center'
  ...
  },
  {
    ...
    description: 'Master the fundamentals of visual design, typography, branding, and design tools like Photoshop and Illustrator.',
    institute: {
      id: 'inst4',
  ...
  },
  {
    ...
    description: 'Deep dive into neural networks, deep learning, and AI applications with hands-on projects.',
    institute: {
      id: 'inst3',
      name: 'DataTech Academy',
      logo: 'https://images.unsplash.com/photo-1551288049-bebda4e38f71?w=100&h=100&fit=crop&crop=center'
  ...
  },
  {
    ...
    description: 'Learn the basics of cybersecurity, network security, threat assessment, and security tools.',
    institute: {
      id: 'inst5',
    ...
    maxStudents: 150,
    level: 'intermediate',
    tags: ['Network Security', 'Ethical Hacking', 'Penetration Testing', 'Security Tools'],
    completionRate: 91,
    totalRevenue: 334866,
    language: 'English'
  },
  {
    ...
    description: 'Complete user experience and user interface design course with portfolio projects.',
    institute: {
      id: 'inst4',
      name: 'Creative Arts Institute',
      logo: 'https://images.unsplash.com/photo-1541698444083-023c97d3f4b6?w=100&h=100&fit=crop&crop=center'
...
interface CourseManagementProps {
 ...
 function CourseManagement({ onViewProfile }: CourseManagementProps = {}) {
  ...
  useEffect(() => {
    setCurrentPage(1)
  }, [viewMode, statusFilter, categoryFilter, levelFilter, instituteFilter, searchTerm])

  // Get unique values for filters
  const categories = Array.from(new Set(courses.map(course => course.category)))
  const institutes = Array.from(new Set(courses.map(course => course.institute.name))).sort()
  ...
  const getLevelBadge = (level: string) => {
    switch (level) {
      case 'beginner':
        return <Badge className="bg-blue-50 text-blue-700 border-blue-200">Beginner</Badge>
      case 'intermediate':
        return <Badge className="bg-purple-50 text-purple-700 border-purple-200">Intermediate</Badge>
      case 'advanced':
        return <Badge className="bg-orange-50 text-orange-700 border-orange-200">Advanced</Badge>
      default:
  ...
  const filteredCourses = courses.filter(course => {
    ...
    const matchesInstitute = instituteFilter.length === 0 || instituteFilter.includes(course.institute.name)
    return matchesSearch && matchesStatus && matchesCategory && matchesLevel && matchesInstitute
  })

  // Pagination logic
  const currentItemsPerPage = viewMode === 'table' ? 10 : 9
  const totalPages = Math.ceil(filteredCourses.length / currentItemsPerPage)
  const paginatedCourses = filteredCourses.slice(0, currentPage * currentItemsPerPage)
  ...
  const getStats = () => {
    return {
      ...
      avgRating: courses.length > 0 ? (courses.reduce((sum, course) => sum + course.rating, 0) / courses.length).toFixed(1) : '0',
      totalRevenue: courses.reduce((sum, course) => sum + (course.totalRevenue || 0), 0)
    }
  }
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-2 text-xs text-muted-foreground">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className="truncate">{course.enrollments}/{course.maxStudents} enrolled</span>
                </div>
              </div>
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-xs">{Math.round((course.enrollments / course.maxStudents) * 100)}%</span>
                </div>
                <Progress value={(course.enrollments / course.maxStudents) * 100} className="h-1.5" />
              </div>
            ...
            {paginatedCourses.map((course) => (
              <TableRow key={course.id} className="hover:bg-muted/50">
                ...
                <TableCell>
                  <div className="flex items-center gap-2">
                    <Avatar className="w-6 h-6">
                      <AvatarImage src={course.institute.logo} alt={course.institute.name} />
                      <AvatarFallback className="text-xs">
                        {course.institute.name.substring(0, 2).toUpperCase()}
                      </AvatarFallback>
                    </Avatar>
                    <span className="text-sm truncate max-w-[120px]">{course.institute.name}</span>
                  </div>
                </TableCell>
                <TableCell>
                ...
                <TableCell>
                  ...
                  <Progress value={(course.enrollments / course.maxStudents) * 100} className="w-16 h-1 mt-1" />
                </TableCell>
                <TableCell>
                  <div className="flex items-center gap-1">
              ...
              <div className="flex flex-col gap-4 sm:flex-row sm:items-center">
                ...
                <Popover open={instituteDropdownOpen} onOpenChange={setInstituteDropdownOpen}>
                  ...
                  <PopoverContent className="w-[280px] p-0" align="start">
                    <Command>
                      <CommandInput placeholder="Search institutes..." className="h-9" />
                      <CommandList>
                          ...
                          {institutes.map((institute) => (
                            ...
                            >
                              <Checkbox
                                ...
                                onCheckedChange={() => handleInstituteToggle(institute)}
                              />
            ...
            {instituteFilter.length > 0 && (
              <div className="flex flex-wrap items-center gap-2">
                ...
                {instituteFilter.map((institute) => (
                  <Badge key={institute} variant="secondary" className="flex items-center gap-1">
                    {institute}
                    <X 
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-2 mt-1">
                      <Avatar className="w-6 h-6">
                        <AvatarImage src={selectedCourse.institute.logo} alt={selectedCourse.institute.name} />
                        <AvatarFallback className="text-xs">
                          {selectedCourse.institute.name.substring(0, 2).toUpperCase()}
                        </AvatarFallback>
                      </Avatar>
                      <span className="text-sm">{selectedCourse.institute.name}</span>
                    </div>
                  </div>
                  ...
                  <div>
                    <Label className="text-sm font-medium text-muted-foreground">Duration & Dates</Label>
                    <div className="text-sm mt-1 space-y-1">
                      <p>Duration: {selectedCourse.duration}</p>
                      <p>Start: {new Date(selectedCourse.startDate).toLocaleDateString()}</p>
                      <p>End: {new Date(selectedCourse.endDate).toLocaleDateString()}</p>
                    </div>
                  </div>
                </div>
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <p className="mt-1 text-lg font-semibold">${selectedCourse.price}</p>
                  </div>
                  
                  <div>
                    <Label className="text-sm font-medium text-muted-foreground">Enrollment</Label>
                    <div className="text-sm mt-1">
                      <p>{selectedCourse.enrollments} / {selectedCourse.maxStudents} students</p>
                      <Progress 
                        value={(selectedCourse.enrollments / selectedCourse.maxStudents) * 100} 
                        className="mt-2 h-2"
                      />
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <div className="text-sm mt-1 space-y-1">
                      ...
                      <p>Total Revenue: ${selectedCourse.totalRevenue?.toLocaleString()}</p>
                    </div>
                  </div>
                </div>
              </div>
